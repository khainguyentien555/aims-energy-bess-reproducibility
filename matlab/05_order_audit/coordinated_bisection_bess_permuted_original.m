function [lambdaBus, P_allow_MW, P_BESS_MW, sizingInfo] = coordinated_bisection_bess_permuted_original( ...
    busData0, branchData, cfg, evcsBuses, demandLambda, orderedBuses)
% Original combined V+L sequential mechanics with one change only:
% processing order is supplied explicitly as orderedBuses.
    ...
    if ~combined_feasible(mfull,cfg)
        warning('Full compensation remains infeasible under original combined criterion.');
    end
    ...
    while outer<cfg.maxOuterBisection
        ...
        if combined_feasible(m,cfg), break; end
        ...
        for kk=1:numel(orderedBuses)
            ...
            while abs(lamU-lamL)>cfg.bisectionTol
                ...
                if combined_feasible(mb,cfg), lamL=lamM; else, lamU=lamM; end
            end
            activeScale(j)=lamL;
        end
    ...
    sizingInfo.finalFeasible=combined_feasible(mf,cfg); sizingInfo.finalMetric=mf;
end
function tf=combined_feasible(m,cfg), tf=m.converged && m.Vmin>=cfg.Vthr && m.LmaxPct<=cfg.Lthr; end
